...
from Utils.isaac_utils_45 import scan_rep, rep_utils
    ...
    if my_world.is_playing():
        ...
        if state==0:
            if ik_first_flag:
                ...
                seg_idx_y, seg_idx_x = np.where(instance_seg_annotator.get_data()["data"]==2)
                depth_img = depth_plane_annotator.get_data()
                sample_rate = 1
                # IDX = np.argwhere(depth_img==depth_img).T
                IDX = np.array([seg_idx_y, seg_idx_x])
                ...
                debug_draw_points(PCD.T,size=5,color=[0,1,0])

                target_pos = np.mean(PCD,axis=1)
                debug_draw_points(target_pos[None,:],size=30,color=[1,0,0])
                target_orientation = [90,55,90]#mat_utils.mat_to_euler(mat_utils.dot([np.linalg.inv(robot_tf), mat_utils.rot_z(90), mat_utils.rot_y(0), mat_utils.rot_x(90)]))
                target_orientation = mat_utils.mat_dot( [np.linalg.inv(np.linalg.inv(world_base_tf).dot(robot_tf)), mat_utils.rot_z(90-60) , mat_utils.rot_x(90)  ])
                target_orientation = mat_utils.mat_to_euler(target_orientation, degrees = True)

                target_pos = np.linalg.inv(robot_tf).dot( np.hstack((target_pos,1))[:,None] )[:3].T[0]
                # target_pos = np.array([0.35762588,   0.0638123,   -0.18280847])
