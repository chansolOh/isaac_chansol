...
def gripper_open():
    my_robot.apply_action(ArticulationAction(
                            joint_indices=[6,7] ,
                          joint_positions = [0,0]) )
def compute_rrt(target_pos, target_quat):
    rrt.set_end_effector_target(target_pos, target_quat)
    rrt.update_world()
    plan = rrt.compute_path(my_robot_task.get_joint_positions(),None)
    if plan is None:
        print("no plan found, retrying... attempt count : ", rrt_atempt_count)
        return None, None
    else:
        pos,ori = my_robot_task.rrt_plan_to_taskspace(plan, ee_prim_name="OMY_grasp_joint")
        pos, ori = pos[-1], ori[-1]
        return pos, ori
    
# def play_rmpflow(gripper_action ="open"):
#     rmpflow.set_end_effector_target(
#             target_position=pos, target_orientation=ori
#         )
#     rmpflow.update_world()
#     action = articulation_rmpflow.get_next_articulation_action()
#     action.joint_positions[-2:] = [0,0] if gripper_action == "open" else [np.pi/4,np.pi/4]
#     action.joint_velocities[-2:] = [0,0]
...
while True:
    ...
    if my_world.is_playing():
        ...
        # print("current_time : ", round(current_time,5), "start_current_time : ", round(start_current_time,5), "stage : ", stage)
        ...
        #     if rrt_flag:
        #         pos, ori = compute_rrt(pre_step_pos, pre_step_quat)
        #         rrt_index = 0
        #         rrt_flag = False
        ...
        #     if pos_crit<0.01 and ori_crit < 0.05:
        #         stage += 1
        #         rrt_flag = True
        #         compute_target_flag = True
        #         rrt_atempt_count=0
        ...
        #             pos,ori = compute_rrt(target_pos, mat_utils.euler_to_quat(np.array([90,0,candi_yaw]), degrees=True))
        #             if pos is not None:
        ...
        #     if rrt_flag:
        #         pos, ori = compute_rrt(target_pos, target_quat)
        #         rrt_index = 0
        #         rrt_flag = False
        ...
        #     # print("pos_crit : ", pos_crit, "ori_crit : ", ori_crit)
        #     if pos_crit<0.01 and ori_crit < 0.03:
        #         stage += 1
        #         rrt_flag = True
        #         compute_target_flag = True
        #         rrt_atempt_count=0
