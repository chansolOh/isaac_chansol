...
class IK_isaac:
    ...
    #     self.world_thread = threading.Thread(target=self.run_world_loop, daemon=True)
    #     self.world_thread.start()
    # def step_world(self, render=True):
    ...
    def get_ik(self, 
        ...
        target_joint_positions = self.my_robot_task.compute_ik_traj(target_position = target_pos,
                                            ...
                                            init_joint_state = init_joint_state,
                                            return_traj=return_traj
                                            )
        deg = np.round(np.array(target_joint_positions)/np.pi*180,3)

        # ####3 debug
        # target_joint = target_joint_positions[-1]
        # self.my_robot.apply_action(ArticulationAction(
...
class IsaacIkServer(TcpServer):
    ...
    def default_handler(self, request: Dict[str, Any]) -> Any:
        ...
        if op == "get_ik":
            ...
            init_joint_state = np.array(data.get("init_joint_state")) if "init_joint_state" in data else None
...
if __name__ == "__main__":
    ...
    server = IsaacIkServer(host="0.0.0.0", port=9111, allowed_names=allowed)
    server.start()
