...
import rclpy
import threading
from vla_interfaces.msg import ActionMsg , ObservationMsg
...
class VLA_Ros:
    def __init__(self):
        ...
        obs_pub_node = rclpy.create_node('obs_pub')
        action_sub_node = rclpy.create_node('action_sub')

        self.obs_pub = obs_pub_node.create_publisher(ObservationMsg, '/observation', 10)
        self.action_sub = action_sub_node.create_subscription(
                    ActionMsg, '/action', self.sub_result, 10)


        executor = MultiThreadedExecutor()
        executor.add_node(obs_pub_node)
    ...
    def sub_result(self, msg):
        self.chunk_size = msg.chunk_size
        self.action_size = msg.action_size
        self.action = np.array(msg.data).reshape((self.chunk_size, self.action_size))


    
    def pub_obs(self, state, description):
        obs_msg = ObservationMsg()
        obs_msg.state = state
        obs_msg.description = description
