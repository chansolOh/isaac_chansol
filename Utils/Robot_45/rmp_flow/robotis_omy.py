    ...
    def __init__(
        ...
        if not prim.IsValid():
            if usd_path:
                ...
                usd_path = "/nas/ochansol/isaac/sim2real/Robotis_Robotiq2f140.usd"
                add_reference_to_stage(usd_path=usd_path, prim_path=prim_path)
            if self._end_effector_prim_name is None:
                self._end_effector_prim_path = prim_path + "/Robotiq_2F_140/robotiq_base_link"
            else:
                self._end_effector_prim_path = prim_path + "/" + end_effector_prim_name
            if gripper_dof_names is None:
                gripper_dof_names = ["finger_joint_right", "finger_joint_left"]
            if gripper_open_position is None:
                gripper_open_position = np.array([45, 45]) / get_stage_units()
