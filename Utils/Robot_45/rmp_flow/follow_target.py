...
from .isaac_robot import IsaacRobot
...
class FollowTarget(tasks.FollowTarget):
    ...
    def set_robot(self) -> IsaacRobot:
        """[summary]

        Returns:
            IsaacRobot: [description]
        """
        if self._robot_prim_path is None:
            self._robot_prim_path = find_unique_string_name(
                initial_name="/World/IsaacRobot", is_unique_fn=lambda x: not is_prim_path_valid(x)
            )
        if self._robot_name is None:
            self._robot_name = find_unique_string_name(
                initial_name="my_robot", is_unique_fn=lambda x: not self.scene.object_exists(x)
            )
        return IsaacRobot(prim_path=self._robot_prim_path, name=self._robot_name)
